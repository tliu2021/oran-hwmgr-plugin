...
func Authenticator(handler authenticator.Request) api.Middleware {
	return func(next http.Handler) http.Handler {
		return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			response, ok, err := handler.AuthenticateRequest(req)
			if err != nil {
				api.ProblemDetails(w, fmt.Sprintf("failed to authenticate request: %v", err), http.StatusUnauthorized)
				return
			}

			if !ok {
				api.ProblemDetails(w, "unable to authenticate request", http.StatusUnauthorized)
				return
			}
...
func Authorizer(kubernetesAuthorizer authorizer.Authorizer) api.Middleware {
	return func(next http.Handler) http.Handler {
		return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			...
			if !ok {
				api.ProblemDetails(w, "user not in context", http.StatusBadRequest)
				return
			}
